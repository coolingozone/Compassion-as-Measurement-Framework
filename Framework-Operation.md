## Compassion as Measurement Operation Process
### Core operational assumption
Judgment outcomes are not shaped primarily by selected options.
判断结果并非主要由所选选项决定。
They are shaped by the measurement intervention applied to the possibility space.
Operational Process（操作流程）
This document describes the minimal executable procedure of the
...
界定当前可行选项集合与结构性限制。
This must include external forces (他力), such as:
必须包含外部力量（他力），例如：
●	decisions by others
 他人的决定
●	institutional or procedural constraints
 制度或流程限制
●	resource or time limitations
 资源或时间窗口限制
External forces are treated as structural conditions of the possibility space, not as noise.
他力被视为可能性空间的结构条件，而不是噪音。
________________________________________
### Step 2 — Identify dominant noise
Identify the dominant noise affecting judgment at this moment, such as:
识别当前影响判断的主要噪音，例如：
●	fear（恐惧）
●	urgency（急迫感）
●	loss aversion（损失厌恶）
●	emotional amplification（情绪放大）
